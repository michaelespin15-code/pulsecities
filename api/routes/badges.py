...
def _badge_svg(zip_code: str, name: str | None, score: float | None) -> str:
  ...
  <rect width="320" height="64" rx="8" fill="#111823" stroke="#303c4c" stroke-width="1"/>
  <rect x="12" y="16" width="32" height="32" rx="6" fill="#1a1a2e"/>
  <polyline points="14,32 19,32 22,25 25,39 28,29 31,35 34,32 42,32" fill="none" stroke="#ed6317" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
  <text x="56" y="26" font-family="{_SANS}" font-size="12" font-weight="600" fill="#e4e8ec">{e_zip}{(' ' + chr(183) + ' ' + place) if place else ''}</text>
  <text x="56" y="44" font-family="{_MONO}" font-size="8.5" letter-spacing="0.4" fill="{tier_color}">{tier_text}</text>
  <text x="308" y="32" text-anchor="end" font-family="{_MONO}" font-weight="600"><tspan font-size="20" fill="{tier_color}">{score_str}</tspan><tspan font-size="10" fill="#93a1ad">/100</tspan></text>
  <text x="308" y="48" text-anchor="end" font-family="{_MONO}" font-size="9" fill="#93a1ad">pulsecities.com</text>
</svg>'''
